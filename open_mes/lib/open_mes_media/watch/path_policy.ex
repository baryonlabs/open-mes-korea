defmodule OpenMes.Media.Watch.PathPolicy do
  ...
  # 디렉터리 세그먼트가 있을 때(= 첫 세그먼트 뒤에 최소 1개 더 = 첫 세그먼트가 파일명이 아닌
  # 디렉터리)에만 첫 세그먼트를 설비로 추정한다. 규약(media_type 세그먼트)은 미충족(false).
  defp parse_segments([eqp, _next | _rest]) when is_binary(eqp) and eqp != "" do
    {eqp, nil, nil, false}
  end

  # 세그먼트가 파일명 하나뿐(디렉터리 구조 없음)이면 설비를 추정할 근거가 없다.
  # 파일명을 equipment_id 로 오인하지 않고 unknown 으로 등록한다(데이터는 meta 에 보존).
