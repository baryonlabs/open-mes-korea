...
#
#     PHX_SERVER=true bin/open_mes start
#
# Alternatively, you can use `mix phx.gen.release` to generate a `bin/server`
# script that automatically sets the env var above.
if System.get_env("PHX_SERVER") do
  config :open_mes, OpenMesWeb.Endpoint, server: true
end
...
if config_env() == :prod do
  ...
  config :open_mes, :dns_cluster_query, System.get_env("DNS_CLUSTER_QUERY")
...
# ── 확장 게이트 (모든 환경 공통 — 환경변수로 켜고 끈다) ─────────────────────
# 이 블록은 config_env 조건 밖에 두어 dev/prod 모두 환경변수로 제어 가능하게 한다.
# 환경변수가 없으면 config.exs/dev.exs 의 기본값이 유지되도록, 명시 설정된 경우에만 덮어쓴다.
# (단순화를 위해 여기서는 환경변수 기본값을 그대로 적용한다 — 켜려면 해당 인프라 필요)

# ── EXT-1 설비 수집 (TimescaleDB 필요) ─────────────────────────────────────
if System.get_env("INGEST_ENABLED") do
...
if System.get_env("MEDIA_ENABLED") do
