...
- AI 조회, 제안과 승인 기반 액션
- DureClaw 분산 오케스트레이션 연동 허브(로컬·원격 에이전트 협업)

특정 공장만 필요한 기능은 코어에 계속 추가하지 않고 명시적인 확장
지점으로 연결합니다.

### 확장 결합 모델 — 별도 repo가 코어 수정 0으로 붙는다

확장은 코어 소스를 건드리지 않고 **deps 한 줄**로 결합합니다. 외부
개발자는 자신의 git repo로 확장을 만들어, 호스트의 `mix.exs`에 의존성
한 줄만 추가하면 됩니다. 코어의 `router.ex`, 카탈로그, 설정 어느 것도
수정하지 않습니다.

이를 위해 코어를 잡고 있던 네 곳의 강결합을 해소했습니다.

| 결합 지점 | 해소 방식 |
|---|---|
| 라우터 | 확장별 하드코딩 분기 → 단일 컴파일타임 마운트 매크로. 확장이 `route_spec/0`으로 자기 라우트를 **데이터로** 선언 |
| 카테고리 | 닫힌 목록 → 개방형 atom. 카탈로그가 미지 카테고리도 렌더 |
| 검증 | 소스 경로 추정 → 컴파일 메타데이터 기반(외부 의존성 자동 대응) |
| 등록 | 명시 목록 → 자동 발견(`:auto`) + override·exclude·manual 안전장치 |

확장은 코어(`:open_mes`)에 의존하지 않고 **얇은 계약 패키지
`open_mes_extension_api`(Phoenix 미의존)** 에만 의존합니다. 데이터는
코어가 노출하는 공개 API로만 접근합니다. 코어 도메인은 확장 시스템을
역참조하지 않습니다(단방향 의존 유지).

```elixir
# 확장 repo: 계약 패키지에만 의존 — 코어(:open_mes) 무의존
defmodule MyMesOeePro.Extension do
  ...
  # 라우트를 데이터로 선언 — 코어 router.ex를 건드리지 않는다
  def route_spec, do: %{scope: "/extensions",
    mount: {MyMesOeeProWeb.OeeProLive, :index, "/oee-pro"}}
end
```

```elixir
# 호스트 open_mes/mix.exs — 유일한 편집은 이 한 줄
{:my_mes_oee_pro, git: "..."}
```

발견된 확장은 `mix ext.list`로 한눈에 보고, `mix ext.verify`로 계약
준수(메타데이터·라우트 형태·소스 무결성)를 검증합니다. DureClaw 연동
허브를 비롯한 기존 확장도 모두 이 방식으로 결합되어 있습니다.
